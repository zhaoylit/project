package com.web.modules.system.controller;
...
public class ResourceController {
	...
	public ModelAndView addOrEditIconInit(ModelAndView model,HttpServletRequest request,HttpServletResponse response) throws NumberFormatException, Exception{
		...
		if(id != 0){
			Map nodeMap = resourceService.selectDictById(id);
	...
	public String addOrEditIcon(HttpServletRequest request,HttpServletResponse response) throws IOException{
		...
		try {
			//查询所有的功能
			json = resourceService.addOrEditDict(params);
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
...
//								String path = request.getSession().getServletContext().getRealPath("/");
								...
								//获取配置文件icon的上传路径
								String iconPath = (String)CustomizedPropertyConfigurer.getContextProperty("icon_path");
								String savePath = resourcePath+fileUploadName;
								...
								//返回文件上传后的访问地址	
								resultMap.put("filePath",iconPath + fileUploadName);
