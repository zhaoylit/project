		...
		try {
			...
			while (fii.hasNext()) {
				...
				if (!fis.isFormField()) {
					...
					//获取当前时间
					String fileName = UrlConvertor.getPathByResourceType("6") + this.fileName;
					OSSUtil.uploadImgInputStream(fileName,fis.openStream());
					SysResourceInfo resourceInfo = new SysResourceInfo();
					resourceInfo.setResourceType("1");
					resourceInfo.setOriginalName(originalName);
					resourceInfo.setFileSize(new Long(fis.openStream().available()));
					resourceInfo.setResourcePath(fileName);
					resourceInfo.setStatus("1");
					resourceInfo.setAddedTime(new Date());
					int rowsCount = resourceInfoMapper.insertSelective(resourceInfo);
