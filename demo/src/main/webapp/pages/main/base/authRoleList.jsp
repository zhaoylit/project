		...
		$(function() {
			...
			/* 条件查询 */
		 	...
		 	$("#edit").bind("click",function(){		
		 		...
		 		if (row){
		 			$('#role').form('load',{'rowKey':row.rowKey,'name':row.name,'status':row.status,'createTime':formatDate(row.createTime)});
		 	...
		 	$("#assign").bind("click",function(){
		 		...
		 		if(row){
		 			window.location.href="role/getRolePrivilegePage.do?rowKey="+row.rowKey;
		 		}else
		 	...
		 	$("#view").bind("click",function(){
		 		...
		 		if(row){
		 			window.location.href="role/getRoleOwnedPrivilegePage.do?rowKey="+row.rowKey;
		 		}else
		 			$.messager.alert('提示', '请先选择角色！', '');	
			}); 
		 	...
		 	$("#assignMenu").bind("click",function(){
		 		...
		 		if(row){
		 			window.location.href="role/getRoleOwnedMenuPage.do?rowKey="+row.rowKey;
		 		}else
		 			$.messager.alert('提示', '请先选择角色！', '');	
			}); 
...
<body>
	...
	<!-- 数据表格 -->
	<table id="roleList" class="easyui-datagrid" title="角色列表" pagination="false" style="width:98%;height:500px"></table>
	<!-- 弹出模态窗口  -->
