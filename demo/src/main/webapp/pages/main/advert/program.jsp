...
// 		    pageList:[5,10,15,20,100],//每页的个数  
			...
			columns:[[
                ...
                {field:'startDate',title:'有效开始时间',width:'10%',align:'center'},
                {field:'endDate',title:'有效结束时间',width:'10%',align:'center'},
				...
				{field:'operator1',title:'操作',width:'10%',align:'center',formatter:function(value,row,index){
					return '<a class="editCls" href="javascript:editProgramItemInit(\''+row.rowKey+'\');"></a>';
				}},		
		    ]],
	...
	function addItem(type){
		...
		var id="";
		var url='<%=path%>/advertiser/addOrEditAdvertNewInit.do';   //给一个初始化添加或者编辑节目单的接口
	 ...
	 function keep(){
		 var ids = [];
			...
			var dialog = $('<div id="synchid"></div>').dialog({
			    ...
			    shadow: true,  
			    modal: true,  
				href:'<%=path%>/advert/synchroProgramInit.do', //同步节目单初始化接口
				onClose:function(){
					$(this).dialog('destroy');
				},
	            buttons : [
	              {
	                ...
	                handler : function() {
	        			...
	        			$.ajax({
                    		type:'post',
                    		async:true,
                    		url:'program/synchProgram.do',
                    		data:{
                    			programId:checkedItems[0].id,
                    			airportCode:$("input[name='airportCode']").val(),
                    			viproomId:$("input[name='viproomId']").val(),
                    			deviceIds:chk_value.join(',')	
                    		},
                    		success:function(data){
                    			if(data.result == "1"){
		...
		function editProgramItemInit(id){
			window.open("resource/programResourceManageInit.do?programId="+id); //初始化节目单管理页面的接口
		}
