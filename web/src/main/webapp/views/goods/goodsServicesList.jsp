...
$(function(){
    $('#tableList').datagrid({
        url:'${ctx}/goodsServices/selectList.do',
        idField:'id',
        pagination:true,
	    pageSize:10,
        ...
        columns:[[
    		...
    		{title:'服务图标',field:'imgPath',width:'15%',formatter:function(value,row,index){
				return '<img style="width:50px;height:50px;" src="'+row.imageUrl+'"/>';
				
			}},
    		{title:'服务名称',field:'serviceName',width:'20%'},
...
function editItemInit(type){
	...
	var dialog = $('<div></div>').dialog({
	    ...
	    shadow: true,  
	    modal: true,  
		href:'${ctx}/goodsServices/addOrEditInit.do?id='+selectId,
		onClose:function(){	
			$(this).dialog('destroy');
		},
        buttons : [
          {
            ...
            handler : function() {
            	...
            	if($form.form("validate")){
            		$.ajax({
                		type:'post',
                		url:'${ctx}/goodsServices/addOrEdit.do',
                		data:$form.serialize(),
...
function deleteItem(){
	...
	$.messager.confirm("确认信息","确定删除选中的"+ids.length+"条数据吗？", function (r) {  
        if (r){  
        	$.ajax({
        		type:'post',
        		url:"${ctx}/goodsServices/deleteByPrimaryKey.do",
        		data:{	
        			ids:ids.join(",")
        		},
        		success:function(data){
	...
	$.messager.confirm("确认信息","对当前搜索结果进行排序？", function (r) {  
        if (r){  
        	var sortDialog = $('<div></div>').dialog({
        	    ...
        	    shadow: true,  
        	    modal: true,  
        		href:'${ctx}/goodsServices/sortInit.do',
        		onClose:function(){	
        			$(this).dialog('destroy');
        		},
                buttons : [
				{
				    ...
				    handler : function() {
				    	...
				    	if(ids == null || ids == ""){
				    		...
				    		$.ajax({
				        		type:'post',
				        		url:'${ctx}/goodsServices/updateSort.do',
				        		data:{
				        			ids: ids
				        		},
				        		success:function(data){
