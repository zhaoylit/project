...
// 		        searchTool+='&nbsp;&nbsp;<a id="search" onclick="reloadTable()" style="width:80px;height:30px;">搜索</a> ';
    ...
    $("#expandAll").bind("click",function(){
    	var checkedItems = $('#treeTable').treegrid('getSelected');
    	if(checkedItems != null){
    		var id  = checkedItems.id;
    		$('#treeTable').treegrid('expandAll',id);
    	}else{
    ...
    $("#collapseAll").bind("click",function(){
    	...
    	if(checkedItems != null){
    		var id  = checkedItems.id;
    		$('#treeTable').treegrid('collapseAll',id);
    	}else{
    		$('#treeTable').treegrid('collapseAll');
    	}
	});
...
function editItemInit(type){
	...
	var dialog = $('<div></div>').dialog({
	    ...
	    shadow: true,  
	    modal: true,  
		href:'${ctx}/goodsCategory/addOrEditInit.do?id='+selectId+'&pid='+selectPid,
		onClose:function(){	
			$(this).dialog('destroy');
		},
        buttons : [
          {
            ...
            handler : function() {
            	...
            	if($form.form("validate")){
            		$.ajax({
                		type:'post',
                		url:'${ctx}/goodsCategory/addOrEdit.do',
                		data:$form.serialize(),
...
function deleteItem(){
	...
	$.messager.confirm("确认信息","确定删除当前节点和下面的所有子节点吗？", function (r) {  
        if (r){  
        	$.ajax({
        		type:'post',
        		url:"${ctx}/goodsCategory/deleteAllChildrenNodebyId.do",
        		data:{
        			ids:checkedItems.id
        		},
        		success:function(data){
...
function reloadTable(){
	$('#treeTable').treegrid('load',{});
}
</script>
</head>
<body class="easyui-layout">
<table id="treeTable" style="width:100%;height:100%;"></table>
</body>
</html>