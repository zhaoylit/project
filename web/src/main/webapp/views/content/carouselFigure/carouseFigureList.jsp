...
$(function(){
    $('#tableList').datagrid({
        url:'${ctx}/carouselFigure/selectList.do',
        idField:'id',
        pagination:true,
	    pageSize:10,
        ...
        columns:[[
    		...
    		{title:'平台',field:'platformShow',width:'20%'},
	    ...
	    onLoadSuccess:function(data){
			 $.post("${ctx }/dict/getDictByKey.do?dictKey=PLATFORM",{},function(data){
	        	data.splice(0,0,{dictValue:"",dictName:"全部"});
	        	$("#platform").combobox({
	        		data:data,  
...
function editItemInit(type){
	...
	var dialog = $('<div></div>').dialog({
	    ...
	    shadow: true,  
	    modal: true,  
		href:'${ctx}/carouselFigure/addOrEditInit.do?id='+selectId,
		onClose:function(){	
			$(this).dialog('destroy');
		},
        buttons : [
          {
            ...
            handler : function() {
            	...
            	if($form.form("validate")){
            		$.ajax({
                		type:'post',
                		url:'${ctx}/carouselFigure/addOrEdit.do',
                		data:$form.serialize(),
...
function deleteItem(){
	...
	$.messager.confirm("确认信息","确定删除选中的"+ids.length+"条数据吗？", function (r) {  
        if (r){  
        	$.ajax({
        		type:'post',
        		url:"${ctx}/carouselFigure/deleteByPrimaryKey.do",
        		data:{	
        			ids:ids.join(",")
        		},
        		success:function(data){
	...
	$.messager.confirm("确认信息","对当前搜索结果进行排序？", function (r) {  
        if (r){  
        	var sortDialog = $('<div></div>').dialog({
        	    ...
        	    shadow: true,  
        	    modal: true,  
        		href:'${ctx}/carouselFigure/sortInit.do?platform=' + $("#platform").combobox('getValue'),
        		onClose:function(){	
        			$(this).dialog('destroy');
        		},
                buttons : [
				{
				    ...
				    handler : function() {
				    	...
				    	if(ids == null || ids == ""){
				    		...
				    		$.ajax({
				        		type:'post',
				        		url:'${ctx}/carouselFigure/updateSort.do',
				        		data:{
				        			ids: ids
				        		},
				        		success:function(data){
